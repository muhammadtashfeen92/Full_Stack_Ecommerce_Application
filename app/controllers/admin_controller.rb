class AdminController < ApplicationController
  ...
  def index
    ...
    @not_fulfilled_orders = Order.where({ fulfilled: false }).order(created_at: :desc).take(5)
    @fulfilled_orders = Order.where({ fulfilled: true }).order(created_at: :desc).take(5)
    @quick_stats = {
    ...
    if @revenue_by_day.count < 7
      days_of_week = ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Suday"]
